...
Pressing the shortcut defined in `shortcut` in `Tools -> Add-ons -> Config` will search Forvo for the word currently in your clipboard. You can alternatively search using the search bar in the window.
...
Changing `lang` in `Config` will change the language searched. `ja` for Japanese, `ko` for Korean, etc.
